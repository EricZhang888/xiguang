...
class Common extends DbMysql {
	...
	*/
    function get_zhanbi() {
        ...
        $sql = "SELECT pc.cat_name, concat( format( (
				SELECT count( 1 ) 
				FROM xg_product a
				WHERE a.cat_id = pc.cat_id ) / ( 
				SELECT count( 1 ) 
				FROM xg_product ) , 2 ) *100, '%'
				) AS zhanbi
				FROM `xg_product_category` pc
				RIGHT JOIN `xg_product_category` p ON p.cat_id = pc.cat_id
