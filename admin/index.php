...
if ($rec == 'default') {
	...
	$sys_info['num_yichuli'] = $dou->num_rows($dou->query("SELECT * FROM " . $dou->table('product') . " where chulijiegou_id = 1"));
	$sys_info['num_weichuli'] = $dou->num_rows($dou->query("SELECT * FROM " . $dou->table('product') . " where chulijiegou_id <> 1"));
	...
	$smarty->assign("product_list_zhanbi", $dou->get_zhanbi());
