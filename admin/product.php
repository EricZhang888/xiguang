...
if ($rec == 'default') {
	...
	else if ($_REQUEST['weichuli']) {
        $where = $where . " AND chulijiegou_id  <> 1 ";
    }
	else if ($_REQUEST['weitongzhi']) {
        $where = $where . " AND chulijiegou_id  = 1 AND tongzhijiegou_id = 0 ";
    }
	else if ($_REQUEST['yiwanjie']) {
        $where = $where . " AND chulijiegou_id  = 1 AND tongzhijiegou_id = 1 ";
    }
	else {
		$where = $where . " AND chulijiegou_id  <> 1 AND tongzhijiegou_id = 0 ";
	}
    ...
    $limit = $dou->pager('product', 20, $page, $page_url, $where, $get);
    
    $sql = "SELECT id, name, cat_id, add_time, jianyi_id, chulijiegou_id, kuaidi_id, tongzhijiegou_id, keywords, huijidanhao, update_time FROM " . $dou->table('product') . $where . " ORDER BY id DESC" . $limit;
    $query = $dou->query($sql);
    while ($row = $dou->fetch_array($query)) {
        $cat_name = $dou->get_one("SELECT cat_name FROM " . $dou->table('product_category') . " WHERE cat_id = '$row[cat_id]'");
		$chuli_name = $dou->get_one("SELECT cat_name FROM " . $dou->table('article_category') . " WHERE cat_id = '$row[jianyi_id]'");
		$kuaidi_name = $dou->get_one("SELECT cat_name FROM xg_kuaidi_category WHERE cat_id = '$row[kuaidi_id]'");
        $add_time = date("Y-m-d", $row['add_time']);
		$update_time = date("Y-m-d", $row['update_time']);
        
        $product_list[] = array (
                "id" => $row['id'],
                "cat_id" => $row['cat_id'],
				"chulijiegou_id" => $row['chulijiegou_id'],
				"kuaidi_id" => $row['kuaidi_id'],
				"keywords" => $row['keywords'],
                "cat_name" => $cat_name,
				"chuli_name" => $chuli_name,
                "name" => $row['name'],
                "add_time" => $add_time,
				"kuaidi_name" => $kuaidi_name,
				"update_time" => $update_time,
				"huijidanhao" => $row['huijidanhao'],
				"tongzhijiegou_id" => $row['tongzhijiegou_id'] 
        );
    }
...
elseif ($rec == 'insert') {
    ...
    $sql = "INSERT INTO " . $dou->table('product') . " (id, cat_id, name, price, defined, content, image ,keywords, add_time, description, dianpu_id, dizhi, jianyi_id, chulijiegou_id, kuaidi_id, tongzhijiegou_id, huijidanhao)" . " VALUES (NULL, '$_POST[cat_id]', '$_POST[name]', '$_POST[price]', '$_POST[defined]', '$_POST[content]', '$file', '$_POST[keywords]', '$add_time', '$_POST[description]', '$_POST[dianpu_id]', '$_POST[dizhi]', '$_POST[jianyi_id]', '$_POST[chulijiegou_id]', '$_POST[kuaidi_id]', '$_POST[tongzhijiegou_id]', '$_POST[huijidanhao]')";
    $dou->query($sql);
...
elseif ($rec == 'update') {
    ...
    $sql = "update " . $dou->table('product') . " SET cat_id = '$_POST[cat_id]', name = '$_POST[name]', price = '$_POST[price]', defined = '$_POST[defined]' ,content = '$_POST[content]'" . $up_file . ", keywords = '$_POST[keywords]', description = '$_POST[description]', dianpu_id='$_POST[dianpu_id]', dizhi='$_POST[dizhi]', jianyi_id='$_POST[jianyi_id]', chulijiegou_id='$_POST[chulijiegou_id]', kuaidi_id='$_POST[kuaidi_id]', update_time=$update_time, tongzhijiegou_id='$_POST[tongzhijiegou_id]', huijidanhao='$_POST[huijidanhao]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
